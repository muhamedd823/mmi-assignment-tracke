...
def show_menu(message):
    ...
    if not data or 'assignments' not in data: return 

    markup = InlineKeyboardMarkup()
    for task in data['assignments']:
        markup.add(InlineKeyboardButton(f"📝 {task['title'][:30]}", callback_data=f"v_{task['assignment_id']}"))

    bot.send_message(message.chat.id, "✅ *MMI System Online*\nSelect an assignment:", reply_markup=markup, parse_mode='Markdown')
...
def handle_details(c):
    ...
    if task:
        ...
        on_time = "\n".join([f"• {x['trainee_name']}" for x in subs['on_time']]) or "None"
        late = "\n".join([f"• {x['trainee_name']} ⏰" for x in subs['late']]) or "None"
        not_sub = "\n".join([f"• {x['trainee_name']}" for x in subs['not_submitted']]) or "None 🎉"
        ...
        report = (
            ...
            f"📥 {stats['submitted_count']} Submissions\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"

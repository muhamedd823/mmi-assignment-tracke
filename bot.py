...
# --- CONFIG ---
BOT_TOKEN = '8679659340:AAEM3Ie2--PjMZ8eozfI6zE5pQJ-SuW1Zwk'
YOUR_CHAT_ID = '7494977999'
# Updated URL with the Secret Key
API_URL = 'https://lms.mersamedia.org/api_assignment_tracking.php?key=MMI_SECRET_2026'

bot = telebot.TeleBot(BOT_TOKEN, threaded=False)

def get_clean_data():
    """Fetches data using Browser Headers and the Security Key"""
    try:
        headers = {
        ...
        print(f"\n--- [ {time.strftime('%H:%M:%S')} ] SERVER CHECK ---")
...
def format_assignment(task):
    ...
    markup.add(InlineKeyboardButton("🔄 Refresh Stats Now", callback_data="refresh"))
    return msg, markup
...
def manual_report(message):
    ...
    if data and data.get('assignments'):
        ...
        bot.send_message(message.chat.id, "⚠️ API Error. Still getting 403? Contact Server Admin.")
...
def refresh_callback(call):
    ...
    if data and data.get('assignments'):
        ...
        try:
            ...
            bot.answer_callback_query(call.id, "No changes.")
...
print("🚀 MMI Tracking System (Secure Mode) is ONLINE...")

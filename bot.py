...
def create_assignment_buttons(assignments):
    ...
    for ass in assignments:
        ...
        if (mins // 1440) <= 5:   # Show only last 5 days
            active_count += 1
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "refresh":
        ...
        await query.edit_message_text("✅ Data Refreshed!", reply_markup=keyboard)
        return
    ...
    if action == "back_to_list":
        keyboard, _ = create_assignment_buttons(assignments)
        await query.edit_message_text("📚 **Active Assignments**\nSelect one:", parse_mode="Markdown", reply_markup=keyboard)
        return
...
if __name__ == "__main__":
    # Keep-alive for Render / similar platforms
    def keep_alive():
        while True:
